<template>
  <div :style='{"width":"1100px","padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.302)","margin":"10px auto","position":"relative","background":"#fff"}'>
    <div class="section-title" :style='{"margin":"10px 0","fontSize":"20px","lineHeight":"54px","color":"#000","textAlign":"center"}'>确认下单</div>
      ...
      handleChange(value, index, goodid) {
        ...
        payClick() {
            ...
            this.tableData.forEach(item => {
              console.log("座位",item.updatezhuowei);
                  //更新座会表
                  item.updatezhuowei.forEach(item0=>{
                                  if(item0.active!='0'){
                                    item0.select = '1';
                                    item0.active = '0';
                                  }
                                  this.$http.post(`zhuowei/update`, item0).then(res => {});
                                })
                // 获取商品详情信息

	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"33.3%","margin":"0 0 20px 0"}' class="upload" v-if="type!='info' && !ro.picture" label="监控" prop="picture">
					<file-upload
						tip="点击上传监控"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"width":"33.3%","margin":"0 0 20px 0"}' class="upload" v-else-if="ruleForm.picture" label="监控" prop="picture">
				...
				<el-form-item :style='{"width":"33.3%","margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="链接" prop="introduction">
					<el-input
				...
				<el-form-item :style='{"width":"33.3%","margin":"0 0 20px 0"}' v-else-if="ruleForm.introduction" label="链接" prop="introduction">
...
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				picture: [
					{ required: true, message: '监控不能为空', trigger: 'blur' },
				],
				content: [
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.content = this.ruleForm.content.replace(reg,'../../../zixishiguanli/upload');
        } else {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiankong/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiankongCrossAddOrUpdateFlag = false;
						 this.parent.search();
